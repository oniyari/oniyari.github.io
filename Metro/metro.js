...
posizioni[metro["rossa"][21]] = {x:39,y:12,linea:"red"};
...
function stampa()
{
	...
	{
		coor.push({x:posizioni[x[f]].x,y:posizioni[x[f]].y});
	}
...
train.src = 'https://d30y9cdsu7xlg0.cloudfront.net/png/10880-200.png';   // load image

function runtrain(coor) 
{
	var canvas = document.getElementById('map');
	var ctx = canvas.getContext('2d'); 
	var x = coor[0].x * 20 - 30;
	var y = coor[0].y * 30 - 65;
	var i = 0;
	//ctx.clearRect(0, 0, canvas.width, canvas.height);  // clear canvas
	ctx.drawImage(train, x,y, 20, 20);
	setTimeout(function()
	{
		i+=100;
		stampa();
		ctx.drawImage(train, x+i,y+i, 20, 20); // draw image
	}, 2000);
